...
public class OsResourceMonitor {
    ...
    private EventNoticeProperties.OsResourceProperties properties;

    public OsResourceMonitor(ThreadPoolTaskScheduler scheduler, NoticeEventListener noticeEventListener,
                             EventNoticeProperties.OsResourceProperties properties) {
    ...
    private void monitor() {
        ...
        log.debug("机器 CPU 信息：型号 {}，{}核{}线程", processor.getProcessorIdentifier().getName(),
    ...
    private void dealFileDesc(FileSystem fileSystem) {
        ...
        String format3 = "机器 文件描述符：上限值 {}，使用率 {}";
        ...
        if ((double)openFileDescriptors / maxFileDescriptors >= properties.getFileDescriptorsUsedNoticeRate()){
            ...
            argsMap.put("事件描述", "机器文件描述符使用率过高（正常区间 0 ~ " + properties.getFileDescriptorsUsedNoticeRate() + "）");
    ...
    private void dealDisk(FileSystem fileSystem) {
        ...
        for (OSFileStore store : fileStores) {
            String format2 = "机器 磁盘信息：{}，总大小 {}，使用率 {} ";
            ...
            if ((double)(store.getTotalSpace() - store.getUsableSpace()) / store.getTotalSpace() >= properties.getDiskUsedNoticeRate()){
                ...
                argsMap.put("事件描述", "机器磁盘使用率过高（正常区间 0 ~ " + properties.getDiskUsedNoticeRate() + "）");
    ...
    private void dealSwap(GlobalMemory memory) {
        ...
        log.debug("机器 交换分区：总大小 {}，已使用 {}，使用率 {}", total, used, usedRate);

        if (virtualMemory.getSwapTotal() >= 0 &&
                (double)virtualMemory.getSwapUsed() / virtualMemory.getSwapTotal() >= properties.getSwapUsedNoticeRate()) {

            Map<String,String> argsMap = new LinkedHashMap<>(4);
            argsMap.put("事件描述", "机器交换分区使用率过高（正常区间 0 ~ " + properties.getSwapUsedNoticeRate() + "）");
    ...
    private void dealMemory(GlobalMemory memory) {
        ...
        if ((double)(memory.getTotal() - memory.getAvailable()) / memory.getTotal() >= properties.getMemoryUsedNoticeRate()){
            ...
            Map<String,String> argsMap = new LinkedHashMap<>(4);
            argsMap.put("事件描述", "机器内存使用率过高（正常区间 0 ~ " + properties.getMemoryUsedNoticeRate() + "）");
    ...
    private void dealProcessor(CentralProcessor processor) {
        ...
        String format = "机器 CPU 使用分布：空闲 {}，内核态 {}，用户态 {}，低优先级用户态 {}，IO等待 {}，中断处理 {}，软中断处理 {}，虚拟系统占用 {}";
        ...
        if (properties.getCpuUsedNoticeRate() > 0) {
            ...
            if ((sum - idle) / sum >= properties.getCpuUsedNoticeRate()) {
                ...
                argsMap.put("事件描述", "机器 CPU 使用率过高（正常区间 0 ~ " + properties.getCpuUsedNoticeRate() + "）");
                argsMap.put("空闲比率", args[0]);
    ...
    private void dealLoadAverage(CentralProcessor processor, int logicalCount) {
        ...
        if (loadAverage[0] > 0) {
            ...
            log.debug("机器 CPU 负载：1分钟 {}，5分钟 {}，15分钟 {}", loadAverage[0], loadAverage[1], loadAverage[2]);
            ...
            if (properties.getCpuLoadAverageNoticeRate() > 0) {
                ...
                if (flag) {
                    Map<String, String> argsMap = new LinkedHashMap<>(4);
                    argsMap.put("事件描述", "机器CPU负载过高（正常区间 0 ~ " + value + "）");
    ...
    private String getRate(double total, double value) {
        return df.format(total == 0 ? 0 : value / total);
    }
}
